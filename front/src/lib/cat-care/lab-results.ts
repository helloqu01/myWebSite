...
const MARKERS: Record<string, MarkerDefinition> = {
  ...
  CREA: { name: "크레아티닌", aliases: ["CREAT", "CREATININE"], explanation: "신장 여과와 관련된 항목입니다. 근육량과 수분 상태의 영향도 있어 한 번의 수치만으로 판단하지 않습니다." },
  SDMA: { name: "SDMA", explanation: "신장 여과 기능을 살펴보는 표지자입니다. 단독 상승만으로 만성 신장질환을 확정하지 않고 반복 검사와 다른 소견을 함께 평가합니다." },
  PHOS: { name: "인", aliases: ["P", "PHOSPHORUS"], explanation: "체내 인 균형을 보는 항목으로 신장 상태, 식이, 다른 전해질 결과와 함께 해석합니다." },
  ...
  TBIL: { name: "총 빌리루빈", aliases: ["BIL", "BIL-T", "T.BILIRUBIN", "TOTAL BILIRUBIN"], explanation: "적혈구 분해와 간·담도 처리 과정에 관련된 항목입니다." },
  ...
  MONO: { name: "단핵구", explanation: "백혈구 감별계수 중 단핵구 수치입니다. 다른 백혈구 수치와 임상 증상을 함께 봅니다." },
  EOS: { name: "호산구", explanation: "백혈구 감별계수 중 호산구 수치입니다. 알레르기·기생충 등 여러 원인과 함께 해석합니다." },
  BASO: { name: "호염구", explanation: "백혈구 감별계수 중 호염구 수치입니다. 단독 결과보다 전체 백혈구 양상을 함께 봅니다." },
  PLT: { name: "혈소판", explanation: "혈액 응고에 관여합니다. 고양이는 검체에서 혈소판 응집이 생겨 자동 측정값이 낮게 나올 수 있어 도말 확인이 중요합니다." },
  TP: { name: "총단백", aliases: ["T-PRO", "T.PROTEIN", "TOTAL PROTEIN"], explanation: "알부민과 글로불린을 포함한 혈중 단백질의 합입니다. 수분 상태, 간, 신장, 염증 상태와 함께 봅니다." },
  ALB: { name: "알부민", aliases: ["ALBUMIN"], explanation: "간에서 만들어지는 주요 혈중 단백질입니다. 영양, 간 합성, 신장·장 손실 등을 함께 평가합니다." },
  GLOB: { name: "글로불린", aliases: ["GLOBULIN"], explanation: "항체를 포함하는 단백질군입니다. 염증과 면역 상태 등 다양한 요인에 따라 변할 수 있습니다." },
  ...
  TG: { name: "중성지방", aliases: ["TRIG", "TRIGLYCERIDE"], explanation: "혈중 지방 수치로 식이, 대사 및 내분비 상태와 함께 해석합니다." },
  NH3: { name: "암모니아", aliases: ["AMMONIA"], explanation: "혈중 암모니아 수치입니다. 간 기능과 검체 처리 상태, 임상 증상을 함께 확인합니다." },
  ...
  UBLOOD: { name: "요잠혈", aliases: ["URINE BLOOD"], explanation: "소변의 혈액 반응입니다. 채뇨 방법과 요침사 결과를 함께 확인합니다." },
  UBIL: { name: "요빌리루빈", aliases: ["URINE BILIRUBIN"], explanation: "소변의 빌리루빈 반응입니다. 혈액검사와 간·담도 상태를 함께 확인합니다." },
  UPRO: { name: "요단백", aliases: ["URINE PROTEIN"], explanation: "소변 단백 반응입니다. 요비중·요침사와 필요 시 UPC를 함께 확인합니다." },
  UWBC: { name: "요백혈구", aliases: ["URINE WBC"], explanation: "소변의 백혈구 수치입니다. 요침사와 배양검사, 채뇨 방법을 함께 확인합니다." },
  UPH: { name: "요 pH", aliases: ["URINE PH"], explanation: "소변의 산성·알칼리성 정도입니다. 식이와 요침사, 결석 위험을 함께 봅니다." },
};

export interface DatedLabResultGroup {
  date: string;
  rawText: string;
  items: LabResultItem[];
}
...
function normalizeOcrLabLine(rawLine: string): string {
  const normalized = rawLine
    ...
    .replace(/\bE0S(?=\s*\()/gi, "EOS")
    .replace(/^S\s*\.\s*G\s*\.\s*\(\s*U\s*\)/i, "USG")
    .replace(/^(?:I|L|1)\s*-(?=\s)/i, "CL")
    .replace(/\s+/g, " ")
    .trim();

  // 이 병원 PDF의 작은 소수점이 OCR에서 빠지는 경우에만 원본 표의 고정 참고치를 복원합니다.
  return normalized
    .replace(/^(T\.?Protein(?:\s*\([^)]*\))?\s+)57-89(?=\s)/i, "$1 5.7-8.9")
    .replace(/^(Albumin(?:\s*\([^)]*\))?\s+)23-40(?=\s)/i, "$1 2.3-4.0")
    .replace(/^(Globulin(?:\s*\([^)]*\))?\s+)(?:2[.,]?8(?:-|~)?5[.,]?1|28-51)(?=\s)/i, "$1 2.8-5.1")
    .replace(/^(Creatinine(?:\s*\([^)]*\))?\s+)08-24(?=\s)/i, "$1 0.8-2.4")
    .replace(/^(Phosphorus(?:\s*\([^)]*\))?\s+)31-75(?=\s)/i, "$1 3.1-7.5")
    .replace(/^(Ca(?:\s*\([^)]*\))?\s+)78-113(?=\s)/i, "$1 7.8-11.3");
}

function urineSpecificCode(upper: string): string | null {
  if (/^GLUCOSE\s*\((?:ARC|U)\)/.test(upper)) return "UGLU";
  if (/^PROTEIN\s*\((?:U|L)\)/.test(upper)) return "UPRO";
  if (/^BILIRUBIN\s*\((?:U|L)\)/.test(upper)) return "UBIL";
  if (/^WBC\s*\((?:U|L)\)/.test(upper)) return "UWBC";
  if (/^PH\s*\((?:U|L)\)/.test(upper)) return "UPH";
  if (/^BLOOD(?:\s*\((?:U|L)\))?\s/.test(upper)) return "UBLOOD";
  return null;
}
...
export function parseLabText(rawText: string): LabResultItem[] {
  ...
  rawText.split(/\r?\n/).forEach(rawLine => {
    ...
    const urineCode = urineSpecificCode(upper);
    const alias = urineCode ? null : aliases.find(candidate => new RegExp(`(^|[^A-Z0-9])${candidate.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}([^A-Z0-9]|$)`).test(upper));
    const generic = alias || urineCode ? null : line.match(/^([A-Za-z][A-Za-z0-9%.-]{1,16})(?:\s*\([^)]*\))?\s+[<>]?\s*(\d+(?:[.,]\d+)?)/);
    if (!alias && !generic && !urineCode) return;

    const code = urineCode ?? (alias ? aliasToCode.get(alias)! : generic![1].toUpperCase());
    if (ignoredGenericCodes.has(code)) return;
    if (found.has(code)) return;
    const markerText = urineCode ? line.match(/^[^\s]+/)?.[0] ?? urineCode : alias ?? generic![1];
    const markerIndex = urineCode ? 0 : alias ? upper.indexOf(alias) : 0;
...
function dateFromLabLine(line: string): string | null {
  if (/\bpage\s*:?\s*\d+\b|오전|오후|\b\d{1,2}:\d{2}(?::\d{2})?\b/i.test(line)) return null;
  const match = line.match(/\b(20\d{2})\s*[.\/_-]\s*(0?[1-9]|1[0-2])\s*[.\/_-]\s*(0?[1-9]|[12]\d|3[01])\b/);
...
/** 여러 검사일이 이어진 병원 PDF를 날짜별 검사값 묶음으로 나눕니다. */
export function parseDatedLabText(rawText: string, fallbackDate = ""): DatedLabResultGroup[] {
  const sections = new Map<string, string[]>();
  let currentDate = "";

  rawText.split(/\r?\n/).forEach(rawLine => {
    const line = rawLine.trim();
    if (!line || /^--- 다음 페이지 ---$|^=== 다음 원본 ===$/.test(line)) return;
    const detectedDate = dateFromLabLine(line);
    if (detectedDate) currentDate = detectedDate;
    const targetDate = currentDate || fallbackDate;
    if (!targetDate) return;
    const lines = sections.get(targetDate) ?? [];
    lines.push(rawLine);
    sections.set(targetDate, lines);
  });

  return [...sections.entries()].map(([date, lines]) => {
    const text = lines.join("\n").trim();
    return { date, rawText: text, items: parseLabText(text) };
  });
}
